...
class BASE_DATA():
    ...
    def __init__(self, args):
        ...
        self.use_vali_keys = args.use_vali_keys
    ...
    def update_train_val_test_keys(self):
        ...
        if self.exp_mode in ["Given", "LOCV"]:
            ...
            if os.path.exists(train_file_name):
                ...
                for index, window in enumerate(self.train_slidingwindows):
                    ...
                    if sub_id not in all_test_keys:
                        if self.use_vali_keys:
                            if sub_id in all_vali_keys:
                                self.vali_window_index.append(index)
                            else:
                ...
                if self.use_vali_keys:
                    with open(vali_file_name, 'wb') as handle:
                        pickle.dump(self.vali_window_index, handle, protocol=pickle.HIGHEST_PROTOCOL)

            if not self.use_vali_keys:
    ...
    def normalization(self, train_vali, test=None, vali=None):
        ...
        if test is None:
            ...
            vali_sensors = vali.iloc[:,1:-1]
            vali_sensors = self.normalizer.normalize(vali_sensors)
            vali_sensors = pd.concat([vali.iloc[:,0],vali_sensors,vali.iloc[:,-1]], axis=1)
            return train_vali_sensors, test_sensors, vali_sensors
